...
import { extractIdentifiersFromCode } from '../ast/ast';
import { isInt, isObject } from '../typeHelper';
...
export function isWidget(entity: Record<string, unknown>) {
  return (
    typeof entity === 'object' && '$type' in entity && entity.$type === 'WIDGET'
  );
}

export function isAction(entity: Record<string, unknown>) {
  return (
